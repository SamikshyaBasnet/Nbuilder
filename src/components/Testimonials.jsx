...
export default function Testimonials() {
  ...
  const handleDot = (i) => { stopAuto(); setCurrent(i); startAuto(); };
  const handlePrev = () => { stopAuto(); setCurrent(c => (c - 1 + TESTIMONIALS.length) % TESTIMONIALS.length); startAuto(); };
  const handleNext = () => { stopAuto(); setCurrent(c => (c + 1) % TESTIMONIALS.length); startAuto(); };
      ...
      <div className="container">
        ...
        <div className="test-header fade-up" ref={r(0)}>
          ...
          <div className="test-nav-btns">
            <button className="test-nav-btn" onClick={handlePrev} aria-label="Previous">←</button>
            <button className="test-nav-btn" onClick={handleNext} aria-label="Next">→</button>
          </div>
        </div>

        {/* Featured testimonial */}
        <div className="test-featured fade-up" ref={r(1)} style={{ transitionDelay: '0.1s' }}>
        ...
        {/* Thumbnails */}
        <div className="test-thumbs fade-up" ref={r(2)} style={{ transitionDelay: '0.2s' }}>
